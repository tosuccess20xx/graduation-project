<?php
	...
	*/
	...
	if (isset($_SESSION['Username'])) {
		...
		if ($action == 'Manage') { // Manage Students Page
			...
			$stmt = $con->prepare("SELECT * FROM users WHERE GroupID = 3 $query ORDER BY UserID DESC");
			...
			<h1 class="text-center"><i class="fas fa-user-graduate fa-fw fa-lg"></i> إدارة الطلاب</h1>
			<div class="container">
						...
						<?php
							foreach($rows as $row) {
								echo "<tr>";
									...
									echo "<td>
										<a href='students.php?action=Edit&userid=" . $row['UserID'] . "' class='btn btn-success'><i class='fa fa-edit'></i> تحديث</a>
										<a href='students.php?action=Delete&userid=" . $row['UserID'] . "' class='btn btn-danger confirm'><i class='fas fa-user-minus'></i> حذف</a>";
										if ($row['RegStatus'] == 0) {
											echo "<a 
													href='students.php?action=Activate&userid=" . $row['UserID'] . "' 
													class='btn btn-info activate confirm'>
													<i class='fa fa-user-check'></i> تفعيل</a>";
										} else {
											echo "<a 
													href='students.php?action=UnActivate&userid=" . $row['UserID'] . "' 
				...
				<a href="students.php?action=Add" class="btn btn-primary btn-lg">
					<i class="fas fa-user-plus fa-fw fa-lg"></i> إضافة طالب جديد
				</a>
				<?php } ?>
			</div>
			...
			<?php } else {
                ...
                if (isset($_GET['page']) && $_GET['page'] == 'Pending') {
					echo '<div class="nice-message">لا يوجد طلاب غير مفعلين لعرض بياناتهم</div>';
                } else {
                    echo '<div class="nice-message">لا يوجد طلاب لعرض بياناتهم</div>';
					echo '<a href="students.php?action=Add" class="btn btn-primary btn-lg">
							<i class="fas fa-user-plus fa-fw fa-lg"></i> إضافة طالب جديد
						</a>';
                }
				echo '</div>';
		...
		<?php 
			...
			<h1 class="text-center"><i class="fas fa-user-graduate fa-fw fa-lg"></i> إضافة طالب جديد</h1>
			<div class="container">
				<div class="panel panel-primary">
					<div class="panel-heading">إضافة طالب جديد</div>
								...
								<form class="form-horizontal" action="?action=Insert" method="POST" enctype="multipart/form-data">
									...
									<div class="form-group form-group-lg">
										<div class="col-sm-offset-3 col-md-9">
											<input type="submit" value="إضافة طالب" class="btn btn-primary btn-lg form-control" />
										</div>
									</div>
									<!-- End Submit Field -->
								</form>
		...
		<?php 
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				echo "<h1 class='text-center'><i class='fas fa-user-graduate fa-fw fa-lg'></i> إضافة طالب</h1>";
				echo "<div class='container'>";
				...
				if (empty($pass)) {
					$formErrors[] = 'كلمة المرور يجب ألا تكون <strong>فارغاً</strong>';
				}
				...
				if (empty($formErrors)) {
					...
					if ($check == 1) {
						...
						$stmt = $con->prepare("INSERT INTO 
												...
												VALUES(:zuser, :zpass, :zmail, :zname, 3, :zsection, :zband, :zavatar, 1, now())");
						$stmt->execute(array(
						...
						$theMsg = '<div class="alert alert-success text-center">تم إضافة الطالب بنجاح !</div>';
			...
			if ($count > 0) { ?>
				...
				<h1 class="text-center"><i class="fas fa-user-graduate fa-fw fa-lg"></i> تحديث بيانات الطالب</h1>
				<div class="container">
					<div class="panel panel-primary">
							...
							<div class="row">
								<div class="col-md-8">
									<form class="form-horizontal" action="?action=Update" method="POST">
										...
										<div class="form-group form-group-lg">
											<div class="col-sm-offset-3 col-md-9">
												<input type="submit" value="تحديث بيانات الطالب" class="btn btn-primary btn-lg form-control" />
											</div>
										</div>
										<!-- End Submit Field -->
									</form>
			...
			echo "<h1 class='text-center'><i class='fas fa-user-graduate fa-fw fa-lg'></i> تحديث بيانات الطالب</h1>";
			echo "<div class='container'>";
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				if (empty($formErrors)) {
					...
					if ($count == 1) {
						...
						$stmt = $con->prepare("UPDATE users SET Username = ?, Email = ?, FullName = ?, Password = ?, SectionID = ?, BandID = ?, Image = ? WHERE UserID = ?");

						$stmt->execute(array($user, $email, $name, $pass, $section, $band, $avatar, $id));

						// Echo Success Message

						$theMsg = '<div class="alert alert-success text-center">تم تحديث بيانات الطالب بنجاح !</div>';
			...
			echo "<h1 class='text-center'><i class='fas fa-user-graduate fa-fw fa-lg'></i> حذف الطالب</h1>";
			echo "<div class='container'>";
				...
				if ($check > 0) {
					...
					$theMsg = '<div class="alert alert-success text-center">تم حذف الطالب بنجاح !</div>';
			...
			echo "<h1 class='text-center'><i class='fas fa-user-graduate fa-fw fa-lg'></i> تفعيل الطالب</h1>";
			echo "<div class='container'>";
				...
				if ($check > 0) {
					...
					$theMsg = '<div class="alert alert-success text-center">تم تفعيل الطالب بنجاح !</div>';
			...
			echo "<h1 class='text-center'><i class='fas fa-user-graduate fa-fw fa-lg'></i> إلغاء تفعيل الطالب</h1>";
			echo "<div class='container'>";
				...
				if ($check > 0) {
					...
					$theMsg = '<div class="alert alert-success text-center">تم إلغاء تفعيل الطالب بنجاح !</div>';
